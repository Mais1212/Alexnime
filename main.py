...
def create_excel(path="Результат.xlsx", anime_data=[""]):
    ...
    # Создаем шрифты
    main_font = xlwt.easyxf('font: height 240,name Fixedsys,colour_index black, bold off,\
        italic off; align: wrap on, vert top, horiz left;\
        borders: top_color black, bottom_color black, right_color black, left_color black,\
        left thin, right thin, top thin, bottom thin;\
        pattern: pattern solid, fore_colour green;')

    anime_font = xlwt.easyxf('font: height 240,name Comic Sans MS ,colour_index black, bold off,\
        italic off; align: wrap on, vert top, horiz left;\
        borders: top_color black, bottom_color black, right_color black, left_color black,\
        left thin, right thin, top thin, bottom thin;\
...
while start_page < end_page:
    ...
    url_page = HOST + "/catalog?page=" + str(start_page)
    response_main_page = requests.get(url_page, headers=HEADERS, timeout=(0.1, 10), proxies=PROXIES)
    ...
    for link in anime_links:
        ...
        response_anime_page = requests.get(HOST + link, headers=HEADERS, proxies=PROXIES)
        soup_anime_page = BeautifulSoup(response_anime_page.content, "html.parser")

        print(link)
        #Что нужно взять со страницы
        ...
        try:
            select_voices = soup_anime_page.select_one(selecror_voices).text
        except AttributeError:
            select_voices = "Не хватает данных"
        
        full_select = [select_name, select_age, select_ganres, select_age_rating, select_rating, select_voices]
